...
- 시간 역행 시스템의 제약 조건은 첫 날 이후로만 되돌아갈 수 있다는 점 외에는 없음
- 저장과 복구는 WAL 방식으로 구현
    - 스냅샷 방식은 매 시점의 전체 상태를 저장해야 하므로 저장 비용이 큼
    - 역행과 재실행이 빈번한 구조에서는 변경 로그를 누적하는 WAL 방식이 더 적합
    - 로그를 기반으로 복구하므로 상태 변화 추적과 재적용이 쉬움
<!-- card: 🛠️ 주요 기능 및 기여 -->
...
- 행성 소유자 UID와 접근자가 동일하지 않을 경우 행성 조작이 불가능한 `Playground` 제공
    - Firebase Authentication으로 소유자와 접근자를 식별하여 보안 강화
    - 여행을 누르면 자신의 행성은 바로 이동되도록 하고, 상대 행성의 키를 입력하면 해당 행성으로 이동되도록 하여 UX를 개선
