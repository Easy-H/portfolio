...
턴과 주사위 값은 RPC를 기반으로 동기화했으며, 주사위 결과는 시드값만 공유해 실제 물리 연산 결과를 맞추는 방식으로 구현했습니다.

제한된 개발 기간에는 먼저 구현할 내용을 인터페이스로 추상화한 뒤, 우선순위가 높은 기능부터 실제로 구현했습니다.
- 우선 최소 기능을 먼저 구현
- 시간이 허락하면 더 높은 완성도를 위한 확장 기능을 교체하거나 추가
- 좌표 이동, 부드러운 이동, 효과가 동반된 이동을 같은 구조 안에서 단계적으로 확장 가능하게 설계
<!-- card: 🛠️ 주요 기능 및 기여 -->
...
- RPC 기반 동기화를 통해 네트워크 환경에서 주사위 결과를 일치시킴
